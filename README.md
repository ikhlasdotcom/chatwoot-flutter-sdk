...
To keep separate threads for the same user (for example, one conversation per order), pass a distinct `conversationContext` such as your `orderId` when calling `ChatwootClient.create`. If your backend already knows the specific conversation id to reuse, set `conversationId` as well and (optionally) provide `conversationCustomAttributes` for tagging new conversations.
...
| conversationId    | null    | int               | Explicit conversation id to use. Helpful when you already know which order-specific conversation to open. Falls back to the first available conversation when not found.                                                                                                                                                                                                                                                                              |
| conversationContext | null  | String            | Optional context key (for example, an orderId) appended to the client instance key so each order keeps its own persisted contact/conversation/messages.                                                                                                                                                                                                                                                                                               |
| conversationCustomAttributes | null | Map<String, dynamic> | Custom attributes attached when the SDK auto-creates a new conversation (for tagging conversations with order metadata).                                                                                                                                                                                                                                                                                                                            |
